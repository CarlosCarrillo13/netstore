...
describe('Category e2e test', () => {
  ...
  it('should load Categories', async () => {
    ...
    expect(await categoryComponentsPage.getTitle()).to.eq('netstoreApp.category.home.title');
  ...
  it('should load create Category page', async () => {
    ...
    expect(await categoryUpdatePage.getPageTitle()).to.eq('netstoreApp.category.home.createOrEditLabel');
  ...
  it('should create and save Categories', async () => {
    ...
    await promise.all([
      ...
      categoryUpdatePage.setUuidInput('uuid'),
      categoryUpdatePage.bussinessSelectLastOption(),
    ]);

    expect(await categoryUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await categoryUpdatePage.getUuidInput()).to.eq('uuid', 'Expected Uuid value to be equals to uuid');
  ...
  it('should delete last Category', async () => {
    ...
    expect(await categoryDeleteDialog.getDialogTitle()).to.eq('netstoreApp.category.delete.question');
