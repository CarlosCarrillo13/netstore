...
import com.net.store.config.KafkaProperties;
...
class NetstoreKafkaResourceIT {
    ...
    void setup() {
        ...
        NetstoreKafkaResource kafkaResource = new NetstoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/netstore-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/netstore-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
